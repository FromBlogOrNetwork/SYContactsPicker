        ...
        if (status == CNAuthorizationStatusDenied || status == CNAuthorizationStatusRestricted) {
            ...
            [store requestAccessForEntityType:CNEntityTypeContacts completionHandler:^(BOOL granted, NSError * _Nullable error) {
                if (!granted) {
                    ...
                    CNContactFetchRequest *request = [[CNContactFetchRequest alloc] initWithKeysToFetch:@[CNContactIdentifierKey, [CNContactFormatter descriptorForRequiredKeysForStyle:CNContactFormatterStyleFullName], CNContactPhoneNumbersKey, CNContactEmailAddressesKey,CNContactPhoneticMiddleNameKey]];
                    ...
                    if (!success) {
                        ...
                        for (CNContact *contact in contacts) {
                            ...
                            for (CNLabeledValue *phoneNumberValue in contact.phoneNumbers) {
                                ...
                                NSLog(@"读取电话号码:%@",phoneNUmber.stringValue);
